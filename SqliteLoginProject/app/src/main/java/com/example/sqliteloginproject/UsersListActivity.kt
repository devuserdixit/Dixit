...
class UsersListActivity : AppCompatActivity() {
    ...
    private var listUsers: MutableList<String>? = null
...
//        recyclerViewUsers.adapter = UsersRecyclerAdapter
        val userList: MutableList<String>? = listUsers// Your list of users
        val adapter = UsersRecyclerAdapter(userList) // Create an instance of the UsersRecyclerAdapter
     ...
     */
    inner class GetDataFromSQLite : AsyncTask<Void, Void, List<String>>() {
        override fun doInBackground(vararg p0: Void?): List<String> {
            val userList: List<User> = databaseHelper.getAllUser()
